...
class _TestSpeechToTextState extends State<TestSpeechToText> {
  ...
  bool _isListening = false;
  String _text =
      'Flutter is an amazing framework. Flutter allows developers to build beautiful applications efficiently. The power of Flutter lies in its simplicity and performance.';
  List<String> _words = [];
  int _currentIndex = -1;
  ...
  void initState() {
    super.initState();
    _words = _text.replaceAll(RegExp(r'[^\w\s]'), '').split(' '); // Xóa dấu câu
    _initializeSpeech();
  }
  ...
  void _processSpeech(String speech) {
    ...
    String lastWord = speech.trim().split(' ').last.toLowerCase();

    for (int i = _currentIndex + 1; i < _words.length; i++) {
      if (_words[i].toLowerCase() == lastWord) {
        setState(() {
          _currentIndex = i;
        });
        return;
          ...
          text: TextSpan(
            ...
            children: List.generate(_words.length, (index) {
              return TextSpan(
                ...
                style: TextStyle(
                  backgroundColor:
                      (_currentIndex == index) ? Colors.amber : null,
                ),
              );
            }),
          ),
